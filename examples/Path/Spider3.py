...
# path = textpath("SPIDER",0, 0, font=fontname)
path = textpath("NodeBox",0, 0, font=fontname)

# try to center the piece
orig,siz = path.bounds
x,y = int(round(orig.x)), int(round(orig.y))
w,h = int(round(siz.width)), int(round(siz.height))
pp( (x,y,w,h) )

size( w+doubleinset, h+doubleinset)
# size( 700, 240 )
background(0.3,0,0.2)
translate( inset-x, inset-y )
...
for contour in path.contours:
    ...
    # pdb.set_trace()
    points_per_contour = int(round( contour.length / density ))
    path_points = list(contour.points( points_per_contour ))
    lines_per_contour = 3 * points_per_contour
    #pp( path_points )
